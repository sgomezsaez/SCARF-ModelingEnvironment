...
%>
	<%if(typeTopology == "alpha"){ %>
				<div class="NodeTemplateShape unselectable <%=nodeTypeCSSName%> <%if (paletteMode){%> hidden<%}%>" id="<%=visualElementId%>" style="left: <%=left%>px; top: <%=top%>px; border-color:yellow; background:#FFFFBB">
			<%} else if(typeTopology == "gamma"){ %>
				<div class="NodeTemplateShape unselectable <%=nodeTypeCSSName%> <%if (paletteMode){%> hidden<%}%>" id="<%=visualElementId%>" style="left: <%=left%>px; top: <%=top%>px; border-color:green; background:#D8FFD5">
			<%} else {%>
